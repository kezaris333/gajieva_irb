...
char* column(const char* num1, const char* num2, int base) {
    ...
    char* result = (char*)malloc((max_len + 2) * sizeof(char)); // для нуля и переноса
    if (result == NULL) {
    ...
    while (i >= 0 || j >= 0 || car > 0) { // работать пока есть цифры для сложения или для переноса
    ...
    // сдвиг результата так, чтобы он начинался с первой значимой цифры
    char* result_trimmed = strdup(result + k + 1);
    free(result);
...
char* sum_numbers_in_base(int base, int count, ...) {
    ...
    return strdup(sum); // возвращаем копию строки, чтобы её можно было освобождать 
}
